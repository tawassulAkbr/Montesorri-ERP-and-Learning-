    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#667085]">Private conversations with your child's teachers</p>
        </div>
      ...
      {totalUnread > 0 && (
        <div className="flex items-center gap-2 p-3 bg-[#E6F4F1] border border-[#B7DDD6] rounded-xl text-xs font-semibold text-[#006B5D] w-fit">
          <MessageSquare size={14} /> {totalUnread} unread message{totalUnread > 1 ? 's' : ''}
        </div>
      )}

      {loading && threads.length === 0 ? (
        <Card><CardContent className="py-12 text-center text-sm text-[#667085]">Loading conversations...</CardContent></Card>
      ) : threads.length === 0 ? (
        <Card>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-bold text-[#101828]">{t.teacherName}</span>
                    {t.unread > 0 && (
                      <span className="w-5 h-5 rounded-full bg-[#006B5D] text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0">
                        {t.unread}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-[#667085] truncate mt-0.5">{t.lastMessage}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-[10px] text-[#667085]">{t.lastAt ? formatDateTime(t.lastAt) : ''}</span>
