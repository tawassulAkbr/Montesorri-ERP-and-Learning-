  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Learning Streak Progress</h1>
          <p className="text-sm text-slate-500">Track your students' daily micro-learning streaks and badges</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">students on a streak</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-bold text-slate-800">{atRisk}</p>
            <p className="text-xs text-slate-400">streaks at risk today</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-bold text-slate-800">{topStreak}</p>
            <p className="text-xs text-slate-400">longest active streak</p>
          </div>
        </div>
      </div>
      ...
      {/* Table */}
      {loading ? (
        <Card><CardContent className="py-12 text-center text-sm text-slate-400">Loading streak data...</CardContent></Card>
      ) : filtered.length === 0 ? (
        <Card><CardContent className="py-12 text-center text-sm text-slate-400">No students found. They'll appear once they start learning.</CardContent></Card>
      ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(r => (
                  <tr key={r.studentId} className={cn('transition-colors', r.atRisk ? 'bg-red-50/50 hover:bg-red-50' : 'hover:bg-slate-50/70')}>
                    <td className="p-3.5 pl-5 font-semibold text-slate-800">{r.name}</td>
                    <td className="p-3.5 text-slate-500">{r.class.replace('Montessori ', '').replace(' (', ' · ').replace(')', '')}</td>
                    ...
                    <td className="p-3.5 text-center text-slate-600 font-semibold">{r.longestStreak}</td>
                    <td className="p-3.5 text-center text-indigo-600 font-semibold">{r.totalXp}</td>
                    <td className="p-3.5 text-center">
                      <span className="inline-flex items-center gap-1 text-amber-600 font-semibold"><Star size={12} /> Lv {r.level}</span>
                    </td>
                    <td className="p-3.5 text-center text-slate-600 font-semibold">{r.badgeCount}</td>
                    <td className="p-3.5 text-slate-500">{r.lastActivityDate ? formatDate(r.lastActivityDate) : '—'}</td>
                    <td className="p-3.5 pr-5 text-right">
                      {r.atRisk ? (
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 font-bold text-[10px]">Not started</span>
                      )}
                    </td>
                  </tr>
                ))}
