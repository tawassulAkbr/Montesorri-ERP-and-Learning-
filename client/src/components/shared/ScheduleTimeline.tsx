...
const CATEGORY_CONFIG: Record<ScheduleCategory, { label: string; icon: React.ReactNode; bg: string; text: string; border: string }> = {
  ...
  sensorial: { label: 'Montessori Work Period', icon: <Compass size={15} />, bg: 'bg-[#E6F4F1]', text: 'text-[#006B5D]', border: 'border-[#B7DDD6]' },
  phonics: { label: 'Phonics & Reading', icon: <BookOpen size={15} />, bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200' },
  math: { label: 'Early Math Discovery', icon: <BookOpen size={15} />, bg: 'bg-[#E6F4F1]', text: 'text-[#006B5D]', border: 'border-[#B7DDD6]' },
        ...
        return (
          <div
            ...
            className={`p-4 rounded-2xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
              isCurrent
                ? 'bg-gradient-to-r from-[#E6F4F1]/90 to-white border-[#B7DDD6] shadow-sm ring-2 ring-[#B7DDD6]'
          ...
          >
            <div className="flex items-start gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isCurrent && (
                    <span className="text-[10px] font-bold bg-[#006B5D] text-white px-2 py-0.5 rounded-full animate-pulse">
                      ● Active Now
                    </span>
                  )}
                  {item.isLive && (
                ...
                <h3 className="text-sm font-bold text-[#101828] mt-1">{item.title}</h3>
                <p className="text-xs text-[#667085] mt-0.5">{item.description}</p>
                <p className="text-[11px] text-[#667085] mt-1">Lead: {item.teacherName} • {item.class}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:flex-col sm:items-end justify-between border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-100">
              <div className="flex items-center gap-1.5 font-mono text-xs font-bold text-[#344054] bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                <Clock size={13} className="text-[#006B5D]" />
              ...
              <div className="flex items-center gap-1.5">
                ...
                {canDelete && onDelete && (
                  <Button variant="ghost" size="icon" onClick={() => onDelete(item.id)} className="h-7 w-7 text-[#667085] hover:text-red-600">
                    <Trash2 size={14} />
                  </Button>
                )}
              </div>
