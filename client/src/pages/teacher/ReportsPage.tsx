  ...
  if (!selectedStudent) {
    return (
      <div className="p-10 text-center bg-white rounded-2xl border border-slate-100">
        <p className="text-sm font-semibold text-[#344054]">No students enrolled yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#101828]">Student Reports & Evaluation</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <CardHeader className="pb-3">
            ...
            <div className="pt-2">
              <input
                ...
                className="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#006B5D]"
              />
            </div>
          </CardHeader>
            ...
            {filteredStudents.map(student => (
              ...
              >
                <div>
                  ...
                  <p className="text-[10px] text-[#667085] font-mono">{student.class} • Roll #{student.rollNo}</p>
                </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="bg-gradient-to-r from-[#E6F4F1]0 to-[#007A6B] text-white border-0 shadow-sm">
            <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-xs uppercase tracking-wider text-[#006B5D] font-bold">Academic Evaluation</span>
              ...
              <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-4 py-3 rounded-xl">
                <div>
                  <span className="text-[10px] text-[#006B5D] uppercase font-bold block">Overall Standing</span>
          ...
          <Card>
            ...
            <CardContent>
              {studentResults.length === 0 ? (
                <p className="text-xs text-[#667085] py-4 text-center">No recorded test results for this student yet.</p>
              ) : (
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {studentResults.map(res => (
                        <tr key={res.id}>
                          <td className="p-2.5 font-semibold text-[#101828]">{res.testTitle}</td>
                          <td className="p-2.5 text-[#344054]">{res.subject}</td>
                          <td className="p-2.5 font-bold text-[#006B5D]">{res.marksObtained} / {res.maxMarks}</td>
                          <td className="p-2.5">
                            <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${getGradeColor(res.grade)}`}>
                              {res.grade}
                            </span>
                          </td>
                          <td className="p-2.5 text-[#667085] italic text-[11px]">{res.teacherComment || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
