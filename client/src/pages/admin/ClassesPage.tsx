...
export const AdminClassesPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#101828]">Montessori Cohorts</h1>
        <p className="text-sm text-[#667085]">Live overview of each cohort — enrollment, assigned guides, and fee status</p>
      </div>
          ...
          return (
            <Card key={cohort} className="border border-slate-100 shadow-sm overflow-hidden flex flex-col justify-between">
              <CardHeader className="p-5 bg-slate-50/70 border-b border-slate-100 flex flex-row items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-[#101828]">{cohort}</h3>
                  <p className="text-xs text-[#667085]">
                    {cohortTeachers.length} guide{cohortTeachers.length !== 1 ? 's' : ''} assigned
                  </p>
                </div>
                <Badge className="bg-[#006B5D] text-white text-xs">
              ...
              <CardContent className="p-5 space-y-3">
                <div>
                  <span className="text-[11px] font-semibold text-[#667085] uppercase block mb-1.5">
                    Assigned Guides
                  </span>
                  {cohortTeachers.length === 0 ? (
                    <p className="text-xs text-[#667085]">No guides assigned yet.</p>
                  ) : (
                    <div className="space-y-1.5">
                      {cohortTeachers.map(t => (
                        <div key={t.id} className="flex items-center gap-2 text-xs">
                          <div className="w-6 h-6 rounded-full bg-[#E6F4F1] text-[#006B5D] flex items-center justify-center">
                            <GraduationCap size={12} />
                          </div>
                          <span className="text-[#344054] font-medium">{t.name}</span>
                          <span className="text-[#667085] text-[10px] truncate">{t.subject}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div className="bg-slate-50 rounded-lg p-2.5 text-center">
                    <p className="text-lg font-bold text-[#101828]">{scheduleCount}</p>
                    <p className="text-[10px] text-[#667085]">Daily activities</p>
                  </div>
              ...
              <div className="p-4 bg-slate-50/40 border-t border-slate-100 flex items-center justify-between">
                ...
                <span className="text-xs text-[#667085] font-medium">{cohortStudents.length} enrolled</span>
              </div>
            </Card>
          );
        })}
